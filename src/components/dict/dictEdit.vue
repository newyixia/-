<template>
  <div class="dictEdit">
    <el-dialog title="修改字典" :visible.sync="thisType" @close="fun(tname)">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      upUser() {
        var flag=true;
        this.$http.put(http + dictAdd+`?dictValues=${this.strstatus(this.status)}`, {
...
//					this.tablerow[i] = this.tablerow[i].toString()
        ...
        this.form = json
        this.status=[];
        this.showstatus(this.tablerow.detail)
      }
    }
  }
</script>
